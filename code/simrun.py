...
for i in range(N):
    T_new = T + X[i]
    U_Sharpe = B_U * np.exp((-Ea_U[i]/k) * ((1/T_new)-(1/Tref)))/(1 + (Ea_U[i]/(Ea_D - Ea_U[i])) * np.exp(Ea_D/k * (1/T_pk_U[i] - 1/T_new))) 
...
for i in range(N):
    T_new = T + X[i]
    R_Sharpe = B_R * np.exp((-Ea_R[i]/k) * ((1/T_new)-(1/Tref)))/(1 + (Ea_R[i]/(Ea_D - Ea_R[i])) * np.exp(Ea_D/k * (1/T_pk_R[i] - 1/T_new))) 
