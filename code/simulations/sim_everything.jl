...
@time for i in range(0, stop = num_temps-1, length = num_temps)
        T = 273.15 + i 
        all = Float64[]; ϵ_sur = Float64[]; ϵ_ext = Float64[]; feas = Float64[]; #Eϵ_sur = Float64[]; Eϵ_ext = Float64[]; 
        ...
        for j in 1:50
            ...
            ###
            push!(all, sum(bm.>1e-7)); push!(feas, N_sur)
        ...
        rich = mean(all); feas_rich = mean(feas)
        everything[Int(i+1),:] = [rich, std(all)/sqrt(length(all)), feas_rich, std(feas)/sqrt(length(feas)), 
...
col_names = ["richness", "richness_err", "feas", "feas_err", "ϵ_sur_mean", "ϵ_sur_err", "ϵ_ext_mean", "ϵ_ext_err", "ϵ_var_mean", "ϵ_var_err",
...
Temp_rich = range(0, num_temps-1, length = num_temps)
f = Figure(fontsize = 35, resolution = (1200, 900));
ax1 = Axis(f[1,1], xlabel = "Temperature (°C)", ylabel = "MCM Richness", xlabelsize = 50, ylabelsize = 50, ygridvisible = false, xgridvisible = false)
ax2 = Axis(f[1,1], xlabel = "Temperature (°C)", ylabel = "EGLV Feasibility", xlabelsize = 50, ylabelsize = 50, yaxisposition = :right, yticklabelalign = (:left, :center), xticklabelsvisible = false, xlabelvisible = false)
...
linkxaxes!(ax1,ax2)
l1 = [LineElement(color = ("#6B8EDE",0.8), linestyle = nothing, linewidth = 5)]
l2 = [LineElement(color = ("#283747", 0.8), linestyle = nothing, linewidth = 5)]
Legend(f[1,1], [l1, l2], tellheight = false, tellwidth = false, ["MCM Richness", "EGLV Feasibility"], halign = :left, valign = :top)
# Label(f[1,1, TopLeft()], "(a)")
f
